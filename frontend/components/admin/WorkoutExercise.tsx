...
const WorkoutExerciseList: React.FC = () => {
  ...
  const getAllData = () => {
    fetchWorkoutExercises(page + 1, rowsPerPage, search, sortBy, sortOrder);
  };
  ...
  const fetchWorkoutExercises = async (
    ...
    try {
      const response = await axiosInstance.get(apiUrl.WORKOUT_EXERCISES, {
        params: { page, limit, search: searchVal, sort, order },
      });
      if (response?.status == 200) {
        setWorkoutExercises(response.data.data.workoutExercises);
