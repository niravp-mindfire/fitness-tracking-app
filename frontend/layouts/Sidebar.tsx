...
const Sidebar: React.FC<any> = ({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 bg-[#0D47A1] text-white">
        ...
        {menuItems.map((item) => (
          <div key={item.text} className="relative">
            <a
              ...
              className={`flex items-center p-2 cursor-pointer transition-colors duration-200 rounded-lg text-gray-700 hover:bg-[#42A5F5] hover:text-white ${
                pathname === item.path ? 'bg-[#0D47A1] text-white' : ''
              }`}
            >
              <span
