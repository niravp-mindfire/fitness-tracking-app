...
>(
  ...
  async (
    { page, limit, search = '', sort = 'date', order = 'desc' },
    { rejectWithValue },
  ) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch nutrition entries',
      );
    }
  },
);

export const fetchNutritionById = createAsyncThunk<
  Nutrition,
  string,
  { rejectValue: string }
>('nutrition/fetchNutritionById', async (id, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      error.response?.data?.message || 'Failed to fetch nutrition entry',
    );
  }
});
...
export const createNutrition = createAsyncThunk<
  ...
  Nutrition,
  { rejectValue: string }
>('nutrition/createNutrition', async (nutritionEntry, { rejectWithValue }) => {
  try {
    const response = await axiosInstance.post(apiUrl.NUTRITION, nutritionEntry);
    return response.data.data;
  } catch (error: any) {
    return rejectWithValue(
      error.response?.data?.message || 'Failed to create nutrition entry',
    );
  }
});
...
export const updateNutrition = createAsyncThunk<
  Nutrition,
  { id: string; nutritionEntry: Nutrition },
  { rejectValue: string }
>(
  ...
  async ({ id, nutritionEntry }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to update nutrition entry',
      );
    }
  },
);
...
export const deleteNutrition = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>('nutrition/deleteNutrition', async (id, { rejectWithValue }) => {
  try {
    await axiosInstance.delete(`${apiUrl.NUTRITION}/${id}`);
    return id;
  } catch (error: any) {
    return rejectWithValue(
      error.response?.data?.message || 'Failed to delete nutrition entry',
    );
  }
});
...
export const selectAllNutritionEntries = (state: RootState) =>
  state.nutrition.nutritionEntries;
export const selectNutritionLoading = (state: RootState) =>
  state.nutrition.loading;
export const selectTotalNutritionEntries = (state: RootState) =>
  state.nutrition.totalCount;
