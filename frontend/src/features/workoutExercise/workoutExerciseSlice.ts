...
>(
  ...
  async (
    { page, limit, search = '', sort = 'createdAt', order = 'asc' },
    { rejectWithValue },
  ) => {
    try {
...
export const createWorkoutExercise = createAsyncThunk<
  WorkoutExercise,
  WorkoutExercise,
  { rejectValue: string }
>(
  ...
  async (workoutExercise, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(
        apiUrl.WORKOUT_EXERCISES,
        workoutExercise,
      );
      return response.data.data;
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data?.message || 'Failed to create workout exercise',
      );
    }
  },
);
...
export const updateWorkoutExercise = createAsyncThunk<
  WorkoutExercise,
  { id: string; workoutExercise: WorkoutExercise },
  { rejectValue: string }
>(
...
export const deleteWorkoutExercise = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>('workoutExercise/deleteWorkoutExercise', async (id, { rejectWithValue }) => {
  try {
    await axiosInstance.delete(`${apiUrl.WORKOUT_EXERCISES}/${id}`);
    return id;
  } catch (error: any) {
    return rejectWithValue(
      error.response?.data?.message || 'Failed to delete workout exercise',
    );
  }
});
...
export const { updateSort, resetCurrentWorkoutExercise } =
  workoutExerciseSlice.actions;

export const selectAllWorkoutExercises = (state: RootState) =>
  state.workoutExercise.workoutExercises;
export const selectWorkoutExerciseLoading = (state: RootState) =>
  state.workoutExercise.loading;
export const selectTotalWorkoutExercises = (state: RootState) =>
  state.workoutExercise.totalCount;
